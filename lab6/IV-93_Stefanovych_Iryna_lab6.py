...
class Lab6:
    def __init__(self, n, m):
        ...
        x1 = [x1min, x1min, x1min, x1min, x1max, x1max, x1max, x1max, -1.73 * deltax1 + x01, 1.73 * deltax1 + x01, x01,
              x01,
              x01, x01, x01]
        x2 = [x2min, x2min, x2max, x2max, x2min, x2min, x2max, x2max, x02, x02, -1.73 * deltax2 + x02,
              1.73 * deltax2 + x02,
              x02, x02, x02]
        ...
        list_for_a = Lab6.round_matrix(list(zip(x1, x2, x3, x1x2, x1x3, x2x3, x1x2x3, x1kv, x2kv, x3kv)))
        ...
        planning_matrix_with_naturalized_coeffs_x.field_names = ['X1', 'X2', 'X3', 'X1X2', 'X1X3', 'X2X3', 'X1X2X3',
                                                                 'X1X1', 'X2X2', 'X3X3']
        planning_matrix_with_naturalized_coeffs_x.add_rows(list_for_a)
        print(planning_matrix_with_naturalized_coeffs_x)

        Y = Lab6.round_matrix(
            [[Lab6.func(list_for_a[j][0], list_for_a[j][1], list_for_a[j][2]) for i in range(m)] for j in range(15)])
    ...
    # f(x1,x2,3) = 5,0+4,7*x1+3,6*x2+6,4*x3+6,8*x1*x1+0,3*x2*x2+5,3*x3*x3+3,2*x1*x2+0,9*x1*x3+2,7*x2*x3+0,1*x1*x2*x3
    # Функція відгуку = f(х1, х2, х3) + random(10) - 5
    @staticmethod
    def func(x1, x2, x3):
        return 5.0 + 4.7 * x1 + 3.6 * x2 + 6.4 * x3 + 6.8 * x1 * x1 + 0.3 * x2 * x2 + 5.3 * x3 * x3 + 3.2 * x1 * x2 + \
            0.9 * x1 * x3 + 2.7 * x2 * x3 + 0.1 * x1 * x2 * x3 + randrange(0, 10) - 5
